    ...
    async fn load_account_group_members(
        ...
        let rows = sqlx::query(
            "select proxy.location_country, proxy.location_region, proxy.location_city, proxy.location_timezone, membership.account_group_id,
                    ...
                    account.next_refresh_at, account.enabled, account.enable_session_keepalive, account.session_keepalive_models, account.session_keepalive_expected_lengths, account.concurrency_limit,
