...
async fn rate_limit_is_per_model_and_does_not_block_the_other_model() {
    ...
    // 后台仍遵守原冷却；新的手动刷新清除旧冷却，但不跳过本次收到的 429。
    store.set_session_keepalive("acct_a", false);
    store.set_session_keepalive("acct_b", false);
    assert!(
        manager
...
async fn cookie_capture_preserves_tickets_expiry_and_success_pruning_after_restart() {
    ...
    store.set_session_keepalive("acct_b", false);
    tokio::select! {
        result = manager.run(CancellationToken::new()) => panic!("后台不应退出：{result:?}"),
        () = tokio::time::sleep(Duration::from_millis(100)) => {},
    }
    for old in before.models {
        let ticket = policy.tickets.load(&id, &old.model).await.unwrap().unwrap();
        assert_eq!(old.expire_at, Some(ticket.expires_at));
    }
    assert_eq!(proxy.received_requests().await.unwrap().len(), 2);
}
    ...
    for length in [0, 291, 293, 312, 8193] {
        ...
        for model in SESSION_KEEPALIVE_MODELS {
            mock_model(
                ...
                "acct_a",
                model,
                response_with_state(&if length >= 6 {
                    format!("gAAAAA{}", "A".repeat(length - 6))
                } else {
                    "A".repeat(length)
                }),
            )
...
#[tokio::test]
async fn tickets_restore_after_restart_manual_refresh_bypasses_fresh_cache_and_fails_closed() {
    let proxy = MockServer::start().await;
    let (store, policy, manager) = fixture(Some(&proxy.uri())).await;
    let account = store.account("acct_a").unwrap();
...
#[tokio::test]
async fn http_200_valid_length_and_prefix_are_required_without_consuming_body() {
...
#[tokio::test]
async fn configured_lengths_are_account_scoped_and_revalidate_existing_tickets() {
    let proxy = MockServer::start().await;
    let (store, _, manager) = fixture(Some(&proxy.uri())).await;
    for (account, length) in [("acct_a", 332), ("acct_b", 356)] {
        store.set_session_models(account, vec!["gpt-6-astra".to_owned()]);
        store.set_session_expected_length(account, Some(length));
        let value = format!("gAAAAA{}", "A".repeat(length as usize - 6));
        mock_model(&proxy, account, "gpt-6-astra", response_with_state(&value)).await;
        let result = manager
            .refresh(&ProviderAccountId::new(account).unwrap())
            .await
            .unwrap();
        ...
        assert!(
            manager
                .rewrite(&store.account(account).unwrap(), &mut req)
                .await
        );
        assert_eq!(req.turn_state.as_deref(), Some(value.as_str()));
    }
    store.set_session_expected_length("acct_a", Some(292));
    assert!(
        !manager
    ...
    assert!(
        manager
            .available(&store.account("acct_b").unwrap(), "gpt-6-astra")
            .await
    );
    store.set_session_expected_length("acct_a", None);
    assert!(
        manager
            .available(&store.account("acct_a").unwrap(), "gpt-6-astra")
            .await
...
async fn retry_limit_stops_manual_and_background_probes_until_manual_recovery() {
    ...
    store.set_session_models("acct_a", vec!["gpt-6-astra".to_owned()]);
    store.set_session_keepalive("acct_b", false);
    mock_model(&proxy, "acct_a", "gpt-6-astra", ResponseTemplate::new(503)).await;
    let id = ProviderAccountId::new("acct_a").unwrap();
    let result = tokio::time::timeout(Duration::from_secs(140), manager.refresh(&id))
        .await
        .unwrap()
        .unwrap();
    assert!(result.models[0].error.as_ref().unwrap().contains("100"));
    assert_eq!(proxy.received_requests().await.unwrap().len(), 100);
    assert!(
        !manager
            .available(&store.account("acct_a").unwrap(), "gpt-6-astra")
            .await
    ...
    assert_eq!(proxy.received_requests().await.unwrap().len(), 100);
    proxy.reset().await;
    mock_model(&proxy, "acct_a", "gpt-6-astra", success("recovered")).await;
    assert!(
        manager.refresh(&id).await.unwrap().models[0]
            .error
            .is_none()
